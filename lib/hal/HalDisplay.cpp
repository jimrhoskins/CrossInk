...
void HalDisplay::displayGrayscaleBase(RefreshMode fallback, bool turnOffScreen) {
  ...
  // sleep cover, a full-screen swap from arbitrary prior content). Without this,
  // the X3 grayscale base takes its gentle differential happy path and the prior
  // home/reader frame ghosts through the soft aa_pre_bw_mid waveform. Forcing a
  // resync makes displayGrayscaleBase clear first, matching
  // displayBuffer(HALF)/displayBuffer(FULL). The reader's FAST path is
  // deliberately left on the differential path so per-page grayscale stays
  // cheap.
